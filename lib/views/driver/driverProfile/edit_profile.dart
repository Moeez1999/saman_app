...
class _EditProfileDriverState extends State<EditProfileDriver> {
      ...
      body: Stack(alignment: Alignment.topCenter, children: [
        ...
        isLoading == true
          ...
          stream: Firestore.instance
              ...
              .snapshots(),
          builder: (context, snapshot) {
             index = snapshot.data.documents[0]['vehicleType'];
            if (snapshot.data == null)
              return Container(
                child: Center(
                  child: CircularProgressIndicator(
                        ...
                        RoundedButton(
                          ...
                          press: () async {
                            profileUpdateFunction(
                                ...
                                ['number'],
                                snapshot.data.documents[0]
                                ['driverLicenseNumber'],snapshot.data.documents[0]
                                ['vehicleRegistrationNumber'],);
                          },
                        ),
                      ],
  ...
  profileUpdateFunction(cnic1,firstName1,lastName1,number1,drivingLincese1,vehcilenumber1) {
    Firestore.instance
        .collection('users')
        .where("userId", isEqualTo: int.parse(userId))
        .getDocuments()
      ...
      Firestore.instance
          ...
          .updateData({
        "CNIC": cnic.text.isEmpty ? cnic1 : cnic.text,
        "firstName": firstName.text.isEmpty ? firstName1 :firstName.text,
        "lastName": lastName.text.isEmpty ? lastName1 :lastName.text,
        "number": number.text.isEmpty
            ? number1
            : selectedCountryCode + number.text,
        "driverLicenseNumber": driverLicenseNumber.text.isEmpty ? drivingLincese1 :driverLicenseNumber.text,
        "vehicleRegistrationNumber": vehicleRegistrationNumber.text.isEmpty ? vehcilenumber1 : vehicleRegistrationNumber.text,
      }).then((value) => {
        cnic.clear(),
        firstName.clear(),
        lastName.clear(),
